...
import { useGetUserByUsername, useGetFollowing } from "@/lib/react-query/queriesAndMutations";
...
const Following = () => {
  ...
  const { data: followers, isPending: isFollowersLoading } = useGetFollowing(profileUser?.$id || "");
  ...
  const followerUsers = followers?.documents.map((doc: any) => doc.following) ?? [];

  return (
    <div className="flex flex-col gap-6 w-full">
      <h2 className="h3-bold md:h2-bold">Following</h2>
      {followerUsers.length === 0 ? (
        <p className="text-light-3">Not following anyone yet.</p>
      ) : (
